...
class NewVisitorTest(StaticLiveServerTestCase):
    ...
    def test_can_start_a_list_for_one_user(self):
        ...
        inputbox = self.browser.find_element_by_id('id_new_item')
        inputbox.send_keys('Use peacock feathers to make a boa')
        inputbox.send_keys(Keys.ENTER)

        # The page updates again, and now shows both items on her list
        self.wait_for_row_in_list_table('1: Buy peacock feathers')
        self.wait_for_row_in_list_table('2: Use peacock feathers to make a boa')
    ...
    def test_multiple_users_can_start_lists_at_different_urls(self):
        # Connie starts a new to-do list
        self.browser.get(self.live_server_url)
        inputbox = self.browser.find_element_by_id('id_new_item')
        inputbox.send_keys('Buy peacock feathers')
        inputbox.send_keys(Keys.ENTER)
        ...
        connie_list_url = self.browser.current_url
        self.assertRegex(connie_list_url, '/lists/.+')



        # Now a new user, Frank, comes along to the site.

        ## We use a new browser session to make sure that no information of Connie's is coming through from cookies, etc
        self.browser.quit()
        self.browser = webdriver.Chrome()

        # Frank visits the home page. There is no sign of Connie's list
        self.browser.get(self.live_server_url)
        page_text = self.browser.find_element_by_tag_name('body').text
        self.assertNotIn('Buy peacock feathers', page_text)
        self.assertNotIn('make a boa', page_text)

        # Frank starts a new list by entering a new item. He is less interesting than Connie...
        inputbox = self.browser.find_element_by_id('id_new_item')
        ...
        self.assertNotEqual(frank_list_url, connie_list_url)

        # Again, there is no trace of Connie's list
        page_text = self.browser.find_element_by_tag_name('body').text
        self.assertNotIn('Buy peacock feathers', page_text)
        self.assertIn('Buy milk', page_text)

        # Satisfied, they both go back to doing whatever



    ## Now we get onto the ~ a e s t h e t i c ~ tests ##

    def test_layout_and_styling(self):
        # Connie goes to the home page
        self.browser.get(self.live_server_url)
